(function () {
  ...
  const c = () => copy[locale];
  function storyCoverSrc(story) { return `assets/stories/${story.id.toLowerCase()}.webp`; }
  const storyCoverAlt = story => locale === "en" ? `${story.title.en} story illustration` : `${story.title.zh}故事插画`;
  ...
  function storyCard(story) {
    ...
    return `<article class="story-card"><button class="card-open" data-open-story="${story.id}"><figure class="story-cover story-cover-card"><img src="${storyCoverSrc(story)}" width="1280" height="560" loading="lazy" alt="${text(storyCoverAlt(story))}" data-story-cover/><span class="score-badge">${story.score}</span></figure><div class="card-body"><div class="story-head"><span class="story-id">${story.id} · ${text(t(story.sourceGroupName))}</span></div><h3>${text(t(story.title))}<small>${locale==="en"?text(story.title.zh):text(story.title.en)}</small></h3><p class="decision">${text(t(story.iconicDecision))}</p><p class="summary">${text(t(story.synopsis))}</p><div class="tags"><span class="tag ${story.priority==="pilot"?"pilot":""}">${text(priorityLabel(story.priority))}</span><span class="tag">${story.ageRange} ${locale==="en"?"years":"岁"}</span><span class="tag">${story.durationTier} ${c().minutes}</span><span class="tag">${text(categoryLabel(story))}</span><span class="tag ${story.hasEmbeddedGames?"accent":""}">${story.hasEmbeddedGames?c().twoGames:c().interactionsOnly}</span></div></div></button><div class="card-footer"><span class="relation-count">${story.mechanicIds.join(" · ")}${story.standaloneGameIds.length?` → ${story.standaloneGameIds.join(" · ")}`:""}</span><button class="compare-toggle ${selected?"selected":""}" data-compare-kind="story" data-compare-id="${story.id}">${selected?c().selected:c().compare}</button></div></article>`;
  }
  ...
  function mechanismName(raw){
    const item=localGame(raw),english=window.AtlasI18n.englishItem(raw);
    return `<span class="embedded-mechanism-name">${text(item.title)}<small>${text(locale==="en"?raw.title:english.title)}</small></span>`;
  }
  function embeddedMechanismVisuals(game){
    const mechanisms=game.mechanicIds.map(id=>gameById.get(id)).filter(Boolean);
    if(!mechanisms.length)return "";
    const layout=mechanisms.length===1?"mechanisms-1":"mechanisms-2";
    return `<div class="embedded-mechanisms ${layout}">${mechanisms.map(raw=>{
      const item=localGame(raw);
      return `<button class="embedded-mechanism" type="button" data-open-game="${raw.id}" aria-label="${text(`${raw.id} · ${item.title}`)}"><span class="embedded-mechanism-image"><img src="assets/concepts/${raw.id.toLowerCase()}.webp" loading="lazy" alt="${text(`${raw.id} · ${item.title}`)}" data-concept-image/><span class="embedded-mechanism-arrow" aria-hidden="true">→</span></span><span class="embedded-mechanism-caption"><span class="embedded-mechanism-id">${raw.id}</span>${mechanismName(raw)}<span class="embedded-mechanism-arrow" aria-hidden="true">→</span></span></button>`;
    }).join("")}</div>`;
  }
  ...
  function openStory(id,resetScroll=true,opener=null){const s=storyById.get(id);if(!s)return;activeDetail={kind:"story",id};$("drawerId").textContent=`${s.id} · ${t(s.sourceGroupName)} · ${priorityLabel(s.priority)}`;
  ...
  document.addEventListener("click",event=>{const story=event.target.closest("[data-open-story]");if(story){openStory(story.dataset.openStory,true,story);return;}const game=event.target.closest("[data-open-game]");if(game){openGame(game.dataset.openGame,true,game);return;}const compare=event.target.closest("[data-compare-id]");if(compare)selectCompare(compare.dataset.compareKind,compare.dataset.compareId);});
  ...
  document.addEventListener("error",event=>{const image=event.target;if(!image||typeof image.matches!=="function")return;if(image.matches("[data-story-cover]")){image.hidden=true;image.closest(".story-cover")?.classList.add("cover-fallback");}if(image.matches("[data-concept-image]"))image.hidden=true;},true);
  applyLanguage();
})();
